...
public class DDUtil {
    ...
    public static void skullHeartBlast(World world, LivingEntity player, Entity attacked) {
        if (player.hasStatusEffect(DDEffects.EXUDATION)) {
            world.getEntitiesByClass(LivingEntity.class, attacked.getBoundingBox().expand(8.0 + player.getStatusEffect(DDEffects.EXUDATION).getAmplifier()), getKnockbackPredicate(player, attacked, true)).forEach(entity -> {
                ...
                entity.addVelocity(vec3d2.x, 0.25F, vec3d2.z);
